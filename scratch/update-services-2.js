...
const DEFAULT_SERVICES = [
  ...
  },
  {
    ...
    differentials_title: 'O que garantimos:',
    differentials: [
      'Projetos personalizados e integration inteligente de sistemas',
...
async function updateServices() {
  try {
    ...
    if (data && Array.isArray(data.value)) {
      console.log('Found existing services in database. Reordering and merging...');
      const dbServices = data.value;
      
      const merged = DEFAULT_SERVICES.map(defS => {
        const dbS = dbServices.find(s => s.id === defS.id);
        if (dbS) {
          console.log(`Retaining database settings for service: ${defS.id}`);
          return { ...defS, ...dbS };
        } else {
          console.log(`Seeding new service: ${defS.id}`);
          return defS;
        }
      });
      services = merged;
    ...
    if (upsertError) {
      ...
      console.log('Services successfully updated and reordered in Supabase database!');
    }
  } catch (err) {
